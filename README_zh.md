...
### 使用共享密钥进行进一步通信
我们还没有完成我们的故事。虽然Alice和Bob就一个共享秘密点达成了一致，但Alice仍然没有约Bob去她非常想要的约会。
...
在已知的迪菲-赫尔曼（DH）协议中，双方公开传输素数`𝑝`和生成元`𝑔`，该生成元在与值`𝑝`对应的群中。Alice随机生成私钥`𝑎`并公开广播她的公钥$𝐴 = 𝑔^𝑎\ \ \ \ (mod\ 𝑝)$。类似地，Bob随机生成私钥`𝑏`并公开广播他的公钥$𝐵 = 𝑔^𝑏\ \ \ \ (mod\ 𝑝)$。然后Alice获取Bob的公钥并将其提升到她的私钥的幂，从而计算值$𝐾 = 𝐵^𝑎 = (𝑔^𝑏)^𝑎 =𝑔^{𝑎𝑏}\ \ \ \ (mod\ 𝑝)$。以同样的方式，Bob计算值$𝐾 = 𝐴^𝑏 = (𝑔^𝑎)^𝑏 =𝑔^{𝑎𝑏}\ \ \ \ (mod\ 𝑝)$。在整个过程结束时，Alice和Bob能够就公共`𝐾`值达成一致，而无需在他们之间传输它。
...
与ECDH协议不同，双方不必事先协调任何事情，在ECDSA协议中，双方必须事先就公钥达成一致。只有在每一方确定他们持有的公钥确实属于他们想要与之通信的人之后，才能使用该协议。否则，用每一方拥有的公钥验证签名就没有意义。
...
这里也存在基于ECDLP的ECDSA和基于DLP的埃尔加马尔之间的明显相似性。在这两种情况下，双方都必须事先协调公钥，并且每次我们想要签署新消息时都需要生成一个随机的`𝑘`值。此外，在这两种情况下，监听双方之间消息的攻击者都无法推断出有用的信息，从而允许他们伪造签名。
...
## 曲线是异常的
如果某条曲线具有这样的性质：曲线的阶（其上的点数）恰好等于模数 `p`，那么它被称为"异常曲线"（Anomalous Curve），并且容易受到称为 Smart 攻击的攻击。这种攻击使用 `p进数`（p-adic numbers）。这样的数可以表示为 `p` 的幂（正和负）与系数的和。形式上，这样的数 `s` 是以下形式的级数：
